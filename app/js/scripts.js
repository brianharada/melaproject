...
function populatePaths() {
    ...
    if ($('#networkdir').val()) {
        pathobj.networkpath = path.join($('#networkdir').val(), 'front-end');
    }

    if ($('#cssdir').val()) {
        pathobj.csspath = path.join($('#cssdir').val(), pathobj.projectname);
    }

    if ($('#jsdir').val()) {
        pathobj.jspath = path.join($('#jsdir').val(), pathobj.projectname);
    }

    if ($('#imagedir').val()) {
        pathobj.imagepath = path.join($('#imagedir').val(), pathobj.projectname);
    }
...
$('#js-createButton').on('click', function () {
    ...
    var checklistpath = pathobj.networkpath.split('500 Web Marketing')[0] + '500 Web Marketing/Design and Front-end/front-end/melaproject-misc/checklist.txt';
